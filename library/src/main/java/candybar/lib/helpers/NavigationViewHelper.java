...
public class NavigationViewHelper {
    ...
    public static void initKustom(NavigationView navigationView) {
        Context context = ContextHelper.getBaseContext(navigationView);
        MenuItem menuItem = navigationView.getMenu().findItem(R.id.navigation_view_kustom);
        if (menuItem == null) return;

        boolean shouldShow = context.getResources().getBoolean(R.bool.enable_kustom) && 
            CandyBarApplication.getConfiguration().hasKustomFolders();
        Log.d("CandyBar", "Kustom menu visibility: " + shouldShow + 
            " (enable_kustom=" + context.getResources().getBoolean(R.bool.enable_kustom) + 
