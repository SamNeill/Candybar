...
public abstract class CandyBarApplication extends Application {
    ...
    private static CandyBarApplication.Configuration mConfiguration;
    ...
    public void attachBaseContext(Context base) {
        ...
        // Initialize locale here before any other UI components
        if (Preferences.get(base).isTimeToSetLanguagePreference()) {
            Preferences.get(base).setLanguagePreference();
        }
    ...
    public void onConfigurationChanged(@NonNull android.content.res.Configuration newConfig) {
        ...
        // Check if system locale changed and update app locale accordingly
        Locale systemLocale;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            systemLocale = newConfig.getLocales().get(0);
        } else {
            systemLocale = newConfig.locale;
        }
        
        // Only update if auto language setting is enabled
        if (Preferences.get(this).isTimeToSetLanguagePreference()) {
            List<Language> languages = LocaleHelper.getAvailableLanguages(this);
            boolean localeUpdated = false;
            
            // Try to find exact match
            for (Language language : languages) {
                Locale l = language.getLocale();
                if (systemLocale.toString().equals(l.toString())) {
                    Preferences.get(this).setCurrentLocale(l.toString());
                    localeUpdated = true;
                    break;
                }
            }
            
            // Try to find language match if no exact match
            if (!localeUpdated) {
                for (Language language : languages) {
                    Locale l = language.getLocale();
                    if (systemLocale.getLanguage().equals(l.getLanguage())) {
                        Preferences.get(this).setCurrentLocale(l.toString());
                        localeUpdated = true;
                        break;
                    }
            ...
            // Apply the locale change if we found a match
            if (localeUpdated) {
                LocaleHelper.setLocale(this);
            }
        }
    ...
    public static class Configuration {
        ...
        private NavigationViewStyle mNavigationViewStyle = NavigationViewStyle.MINI_DRAWER;
        ...
        private boolean hasKustomFolders = false;
        ...
        public Configuration setHasKustomFolders(boolean hasKustomFolders) {
            this.hasKustomFolders = hasKustomFolders;
            return this;
        }
        ...
        public boolean hasKustomFolders() {
            return hasKustomFolders;
        }
    }
