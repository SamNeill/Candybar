...
public class Preferences {
    ...
    public boolean isTimeToShowHomeIntro() {
        Log.d("CandyBar", "Checking isTimeToShowHomeIntro, current value: " +
                getSharedPreferences().getBoolean(KEY_HOME_INTRO, true));
        return getSharedPreferences().getBoolean(KEY_HOME_INTRO, true);
    }

    public void setTimeToShowHomeIntro(boolean bool) {
        Log.d("CandyBar", "Setting isTimeToShowHomeIntro to: " + bool);
    ...
    public void setLanguagePreference() {
        ...
        // Keep auto-language enabled by default
        setTimeToSetLanguagePreference(true);
    }

    public void disableAutoLanguagePreference() {
        setTimeToSetLanguagePreference(false);
    }

    public void enableAutoLanguagePreference() {
        setTimeToSetLanguagePreference(true);
        // Update to current system language immediately
    ...
    public void setRequestConsent(boolean consent) {
        getSharedPreferences().edit().putBoolean(KEY_REQUEST_CONSENT, consent).apply();
    }

    public boolean hasRequestConsent() {
        return getSharedPreferences().getBoolean(KEY_REQUEST_CONSENT, false);
    }

    public boolean isRequestConsentAccepted() {
        return hasRequestConsent();
    }

    public boolean isPrivacyPolicyAccepted() {
        return getSharedPreferences().getBoolean(KEY_PRIVACY_POLICY_ACCEPTED, false);
    }

    public void setPrivacyPolicyAccepted(boolean accepted) {
        getSharedPreferences().edit().putBoolean(KEY_PRIVACY_POLICY_ACCEPTED, accepted).apply();
    }

    public boolean isConsentGiven() {
        return getSharedPreferences().getBoolean(KEY_CONSENT_GIVEN, false);
    }

    public void setConsentGiven(boolean consent) {
        getSharedPreferences().edit().putBoolean(KEY_CONSENT_GIVEN, consent).apply();
    }

    public CandyBarApplication.NavigationViewStyle getNavigationViewStyle() {
        // Get default value from configuration
        boolean defaultIsBottomNav = mContext.getResources().getBoolean(R.bool.enable_bottom_navigation);

        // Get saved value or use default from configuration
        return CandyBarApplication.NavigationViewStyle.values()[
                getSharedPreferences().getInt(KEY_NAVIGATION_VIEW_STYLE,
                        defaultIsBottomNav ?
                                CandyBarApplication.NavigationViewStyle.BOTTOM_NAVIGATION.ordinal() :
                                CandyBarApplication.NavigationViewStyle.MINI_DRAWER.ordinal())];
    }

    public void setNavigationViewStyle(CandyBarApplication.NavigationViewStyle style) {
        // Save the preference first
        getSharedPreferences().edit().putInt(KEY_NAVIGATION_VIEW_STYLE, style.ordinal()).apply();

        // Then update the configuration
        CandyBarApplication.getConfiguration().setNavigationViewStyle(style);
    }

    public boolean isTimeToShowRequestSearchIntro() {
        return getSharedPreferences().getBoolean(KEY_REQUEST_SEARCH_INTRO, true);
    }
