...
public class InAppBillingFragment extends DialogFragment {
    ...
    private MaterialDialog mBillingDialog;
    private List<InAppBilling> mProducts;
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        mBillingDialog = builder.build();
        mBillingDialog.setCancelable(false);
        mBillingDialog.setCanceledOnTouchOutside(false);
        mBillingDialog.show();
        setCancelable(false);

        mInAppList = (ListView) mBillingDialog.findViewById(R.id.inapp_list);
        mProgress = (ProgressBar) mBillingDialog.findViewById(R.id.progress);
    ...
    private void initDonationDialog() {
        MaterialDialog.Builder builder = new MaterialDialog.Builder(requireActivity())
                ...
                .onPositive((dialog, which) -> {
                    ...
                    if (mProducts.size() > 0) {
                        InAppBillingListener listener = (InAppBillingListener) requireActivity();
                        listener.onInAppBillingSelected(mType, mProducts.get(0));
                    }
                });
